# Wiki Navigation

Welcome to the Azure Data Hub & Microsoft Fabric Terraform Accelerator Wiki!

This wiki provides comprehensive documentation for deploying and managing enterprise-grade data analytics infrastructure on Microsoft Azure and Microsoft Fabric using HashiCorp Terraform and PowerShell.

---

## 📚 Wiki Pages

### Getting Started
- **[Home](Home.md)** - Introduction, architecture overview, and technology stack
- **[Getting Started](Getting-Started.md)** - Prerequisites, setup, and quick start guide

### Core Documentation
- **[Core Components](Core-Components.md)** - Detailed documentation for each Azure and Fabric component
- **[PowerShell Automation](PowerShell-Automation.md)** - PowerShell scripts, automation, and best practices
- **[Compliance & Security](Compliance-and-Security.md)** - HIPAA/GDPR compliance automation and security features
- **[Policy-as-Code with OPA](Policy-as-Code-with-OPA.md)** - Open Policy Agent and Conftest implementation

### Best Practices & References
- **[Best Practices](Best-Practices.md)** - Terraform, Azure, PowerShell, and security best practices
- **[References](References.md)** - Comprehensive links to HashiCorp, Microsoft, and community resources

---

## 🚀 Quick Links

### For New Users
1. [Introduction](Home.md#introduction) - Understand what this accelerator does
2. [Prerequisites](Getting-Started.md#prerequisites) - Get the required tools
3. [Quick Start Guide](Getting-Started.md#quick-start-guide) - Deploy your first environment

### For Developers
1. [Core Components](Core-Components.md) - Understand each module
2. [PowerShell Automation](PowerShell-Automation.md) - Learn automation scripts
3. [Best Practices](Best-Practices.md) - Follow recommended patterns

### For Compliance Officers
1. [Compliance & Security](Compliance-and-Security.md) - HIPAA/GDPR automation
2. [Policy-as-Code](Policy-as-Code-with-OPA.md) - Automated governance
3. [Security Best Practices](Best-Practices.md#security-best-practices) - Security guidelines

### For Architects
1. [Architecture Overview](Home.md#architecture-overview) - Solution architecture
2. [Design Patterns](Home.md#design-patterns) - Medallion architecture, IaC, Policy-as-Code
3. [Best Practices](Best-Practices.md) - Production-ready patterns

---

## 🎯 Key Features

✅ **Rapid Deployment** - Deploy complete data analytics infrastructure in minutes  
✅ **Fully Parameterized** - Multi-tenant ready with zero hardcoded values  
✅ **Compliance Automation** - Built-in HIPAA/HITECH and GDPR enforcement  
✅ **Policy-as-Code** - Automated governance with OPA and Conftest  
✅ **Modular Architecture** - Choose only the components you need  
✅ **Zero Licensing Cost** - Built entirely with free tools  
✅ **CI/CD Ready** - Azure DevOps pipeline included  

---

## 📖 Documentation Structure

```
Wiki
├── Home.md                       - Introduction & architecture
├── Getting-Started.md            - Setup & quick start
├── Core-Components.md            - Component documentation
├── PowerShell-Automation.md      - PowerShell scripts & automation
├── Compliance-and-Security.md    - Compliance features
├── Policy-as-Code-with-OPA.md    - OPA/Conftest policies
├── Best-Practices.md             - Recommended patterns
└── References.md                 - External resources
```

---

## 🔗 External Resources

- **Repository**: [GitHub](https://github.com/Club-Innovate/Azure-Data-Hub-Microsoft-Fabric-Terraform-Accelerator)
- **HashiCorp Terraform**: [Documentation](https://developer.hashicorp.com/terraform/docs)
- **Microsoft PowerShell**: [Documentation](https://learn.microsoft.com/powershell/)
- **Microsoft Azure**: [Documentation](https://learn.microsoft.com/azure/)
- **Microsoft Fabric**: [Documentation](https://learn.microsoft.com/fabric/)
- **Open Policy Agent**: [Documentation](https://www.openpolicyagent.org/docs/latest/)

---

## 📧 Support & Contributing

- **Issues**: [GitHub Issues](https://github.com/Club-Innovate/Azure-Data-Hub-Microsoft-Fabric-Terraform-Accelerator/issues)
- **Discussions**: [GitHub Discussions](https://github.com/Club-Innovate/Azure-Data-Hub-Microsoft-Fabric-Terraform-Accelerator/discussions)
- **Author**: Hans Esquivel - *Start a Discussion to reach me*

---

**Last Updated**: February 2026  
**Version**: 1.0

---

[🏠 Return to Repository](https://github.com/Club-Innovate/Azure-Data-Hub-Microsoft-Fabric-Terraform-Accelerator)
